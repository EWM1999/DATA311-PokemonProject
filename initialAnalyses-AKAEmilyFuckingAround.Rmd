...
```{r}
# data<-poke.train[,3:23]
# data[is.na(data)]<-0
# data$hasGender<-factor(data$hasGender)
# data$isLegendary<-factor(data$isLegendary)
# data$Type_1<-factor(data$Type_1)
# data$Type_2<-factor(data$Type_2)
# data$Color<-factor(data$Color)
# data$hasMegaEvolution<-factor(data$hasMegaEvolution)
# data$Egg_Group_1<-factor(data$Egg_Group_1)
# data$Egg_Group_2<-factor(data$Egg_Group_2)
# data$Body_Style<-factor(data$Body_Style)
# linmod2<-lm(Total~., data = data)
# plot(linmod2)
...
kPoke2<-kmeans(pokeDist, 4, nstart=16)
plot(pokeDist, col = kPoke2$cluster)
points(kPoke2$centers, col = 1:4, pch=8, cex=2)
#kAnimals$cluster
table(rownames(poke.train), kPoke2$cluster)
```
