...
```{r}
library(class)
# dim(na.omit(data.frame(poke.train)))
# dim(factor(poke.train[,13]))
# knnrun<-knn.cv(na.omit(data.frame(poke.train)), cl = factor(poke.train[,13]), k = 5, prob = TRUE)
...
# 
# library(class)
# knnrun<-knn.cv(poke.trainNum, cl = poke.train$isLegendary, k = 5, prob = TRUE)
