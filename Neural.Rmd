...
nngend <- nnet(factor(hasGender)~ Attack + Defense + HP + Sp_Atk + Sp_Def + Speed + Catch_Rate + Height_m + Weight_kg, data=gpoke, size=7)
plotnet(nngend)
table(trainset$hasGender, predict(nngend, type="class"))
```
It appears that our neural net is efefctive in predicting hasGender. 
1 hidden layer with 11 nodes appears to overfit the model, 
         1   2
  False  45   0
  True    0 460
  
Instead 1 hidden layer with 7 nodes appears to have a reasonable misclassification without totally overfitting. 
...
mse<-mean((compute(nnGeneration, testset[,6:11])$net.result-testset$Generation)^2)
mse
```
