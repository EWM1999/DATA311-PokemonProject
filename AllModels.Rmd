...
poke.test<-pokemon[-train,]
poke.train<-pokemon[train,]
train.x<-cbind(HP,Attack,Defense,Sp_Atk,Sp_Def,Speed,Height_m,Weight_kg,Catch_Rate)[train,]
test.x<-cbind(HP,Attack,Defense,Sp_Atk,Sp_Def,Speed,Height_m,Weight_kg,Catch_Rate)[-train,]
...
```{r}
knnLegendCV<-knn.cv(cbind(HP,Attack,Defense,Sp_Atk,Sp_Def,Speed,Height_m,Weight_kg,Catch_Rate),pokemon$isLegendary,k=5,p=1)#,method="classification")
...
Looking at the first two principal components, the first has a high loading on most numeric stats.

Several analyses were run using the principal components, none of which proved to be very useful, EXCEPT FOR...
...
##R is such a WEIRD language
#ike i guess its better than spending all day debugging in python and your problem is a double whitespace
#but this is weird
```
...
It appears then that the optimal amount of nodes for our neural network is equal to 5 within 1 hidden layer. We can see that our misclassification for our gpoketest is relatively okay, and appears to be the best balance when we tune our network to 5 nodes. 
