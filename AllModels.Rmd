...
The following models will require the libraries listed below,
```{r}
library(mclust)
library(cluster)
library(dplyr)
...
for(i in 1:6) {
  ...
  numLeg<-count((legendTemp$isLegendary)=='True')[2,2]
  if (is.na(numLeg)) {
    numLeg=0
  }
  print(numLeg)
...
```{r}
lassomod$cvm[lassomod$lambda == lassomod$lambda.min] # MSE of cross validation with best lambda
 
```
MSE = 0.08835933
reference for code: https://stats.stackexchange.com/questions/269534/why-is-mse-of-cross-validation-higher-compared-to-a-linear-model
...
```{r}
pkmn<-data.frame(pokemon, stringsAsFactors = TRUE)
x <- data.matrix(pkmn)[,-c(1,2, 15, 16)]
...
coef(lasimmin)
```
Total, Generation and Catch_Rate are removed...
```{r}
coef(lasim1se)
```
$hasGender = 1.658138 - 1.717024 * 10^-06 * Sp\_Atk - 5.713537 * 10^-04 * Sp\_Def - 0.6509096 * isLegendary -3.443872* 10^-03 * Egg\_Group\_1 + 2.412421e-04 * Egg\_Group\_2 + -1.037037e-04 * Weight\_kg$
...
coef(lasimmin)
```
Only removed Total and hasGender...
```{r}
coef(lasim1se)
```

0.5952540632 - 0.0004248476 Type_2 -0.0010029107 HP + 0.0006922985 Attack + 0.0032618948 isLegendary + 0.0024960139 Egg_Group_1 + 0.0040329244 Egg_Group_2 - 0.0005070587 CatchRate  - 0.0003747370 BodyStyle
...
```{r}
pcapoke <- prcomp(as.matrix(poke[,c(6:11,20:22)]), scale.=TRUE)
summary(pcapoke)
biplot(pcapoke)
```
...
```{r}
poke[order(pcapoke$x[,2], decreasing=TRUE)[1:4] ,]
```

```{r}
poke[order(pcapoke$x[,3], decreasing=TRUE)[1:4] ,]
```

```{r}
poke[order(pcapoke$x[,4], decreasing=TRUE)[1:4] ,]
```

```{r}
poke[order(pcapoke$x[,5], decreasing=TRUE)[1:4] ,]
```
...
OK, so using LDA with PC1 as a predictor is a decent model for predicting isLegendary.
...
```{r}
poke1<-poke[which(hasGender=='True'),]
pcagenpoke <- prcomp(as.matrix(poke1[,c(6:11,20:22)]), scale.=TRUE)
summary(pcagenpoke)
biplot(pcagenpoke)
```
...
```{r}
round(pcagenpoke$rotation[,1:5], 2)
```
...
pcgenleg[1:20,]
leggenlda <- lda(factor(poke1$isLegendary)~pcgenleg[,1]+pcgenleg[,2]+pcgenleg[,3]+pcgenleg[,4]+pcgenleg[,5],data=pcgenleg)
leggenlda
```
...
So LDA using the first five principal components favours the majority group when classifying isLegendary.
...
Using only PC1, we do even worse, NOT VERY USEFUL!

Let's try a linear model, see if PC1 and PC2 are any good at predicting Pr_Male:

```{r}
linmod<-lm(poke1$Pr_Male~pcgenleg[,1]+pcgenleg[,2]+pcgenleg[,3]+pcgenleg[,4]+pcgenleg[,5])
summary(linmod)
linmod<-lm(poke1$Pr_Male~pcgenleg[,1]+pcgenleg[,4])
summary(linmod)
```

Ok, so using backward selection, PC1 and PC4 meet the cutoff for predicting Pr_Male. Let's take another look at PC1 and PC4 and see if we can interpret this.

```{r}
round(pcagenpoke$rotation[,c(1,4)], 2)
```
...
nngend <- nnet(factor(hasGender)~ Attack + Defense + HP + Sp_Atk + Sp_Def + Speed + Catch_Rate + Height_m + Weight_kg, data=gpoke, size=4)
plotnet(nngend)
...
1 hidden layer with 9 nodes appears to overfit the model, 
          1   2
  False  57   0
  True    0 448
